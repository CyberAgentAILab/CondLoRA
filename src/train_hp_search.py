...
from models.load_model import load_initial_model, load_pretrained_model
...
def search_best_model(save_dir: str) -> Tuple[float, str]:
    run_dirs = [
        os.path.join(save_dir, f)
        for f in os.listdir(save_dir)
        if os.path.isdir(os.path.join(save_dir, f)) and "run" in f
    ]
    ...
    for run_dir in run_dirs:
        ckpt_dir = [
            ...
            if os.path.isdir(os.path.join(run_dir, f)) and "checkpoint-" in f
        ][0]
        trainer_state = load_json(f"{ckpt_dir}/trainer_state.json")
        metric = trainer_state["best_metric"]
        ckpt = trainer_state["best_model_checkpoint"]
        if metric > best_metric:
            best_metric = metric
            best_ckpt = ckpt

    return best_metric, best_ckpt
...
if __name__ == "__main__":
    ...
    ], f"Plese select --lora_type from lora, adalora, conditional_lora, and conditional_lora_x"
    ...
    best_trial = trainer.hyperparameter_search(
        ...
        n_trials=5,
        compute_objective=lambda x: x[f"eval_{metric_for_best_model}"],
    )

    del trainer

    best_metric, best_ckpt = search_best_model(args.save_dir)
    print()
    print(f"Best Metric: {best_metric}, Best Checkpoint: {best_ckpt}")
