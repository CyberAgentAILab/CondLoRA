...
PEFT_TYPE_TO_MODEL_MAPPING = {
    ...
    PeftType.ConditionalLORA: ConditionalLoraModel,
    ...
    **Attributes**:
    ...
    def get_nb_trainable_parameters(self):
        ...
        for n, param in self.named_parameters():
            ...
            if self.peft_config["default"].peft_type == "ConditionalLORA" and "lora" in n:
                continue
        ...
        if self.peft_config["default"].peft_type == "ConditionalLORA":
            for key, values in self.linear_for_lora.items():
                if key == "linear_for_lora_x":
                    trainable_params += values.weight.numel()
                    lora_trainable_params += values.weight.numel()
                    continue
                trainable_params += values["linear_for_lora_A"].weight.numel()
                trainable_params += values["linear_for_lora_B"].weight.numel()
                lora_trainable_params += values["linear_for_lora_A"].weight.numel()
                lora_trainable_params += values["linear_for_lora_B"].weight.numel()
            
        return trainable_params, all_param, lora_trainable_params

    def print_trainable_parameters(self):
        """
        Prints the number of trainable parameters in the model.
        """
        trainable_params, all_param, lora_trainable_params = self.get_nb_trainable_parameters()

        print(
            f"trainable params: {trainable_params:,d} || lora trainable params: {lora_trainable_params:,d} || all params: {all_param:,d} || trainable%: {100 * trainable_params / all_param}"
        )
