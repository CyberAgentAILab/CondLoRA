...
class ConditionalLoraModel(BaseTuner):
    """
    ...
    **Attributes**:
    ...
    def __init__(self, model, config, adapter_name, linear_for_lora=None) -> None:
        # key: target module name such as query, value: dict that key and value are lora_A or lora_B and their linear networks
        print("new version")
        self.linear_for_lora = linear_for_lora if linear_for_lora else {}
        super().__init__(model, config, adapter_name)
        
    def _check_new_adapter_config(self, config: ConditionalLoraConfig) -> None:
        """
    ...
    def _create_and_replace(
        ...
        **optionnal_kwargs,
        ...
        if isinstance(target, LoraLayer):
            ...
            new_module, linear_for_lora = self._create_new_module(lora_config, adapter_name, target, target_name, self.linear_for_lora, **kwargs)
            self.linear_for_lora = linear_for_lora
    ...
    @staticmethod
    def _create_new_module(lora_config, adapter_name, target, target_name, linear_for_lora, **kwargs):
        ...
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            if target_name not in linear_for_lora:
                linear_for_lora[target_name] = {
                    "linear_for_lora_A": nn.Linear(in_features=in_features, out_features=lora_config.r, bias=False),
                    "linear_for_lora_B": nn.Linear(in_features=in_features, out_features=lora_config.r, bias=False)
                }
                nn.init.kaiming_uniform_(linear_for_lora[target_name]["linear_for_lora_A"].weight, a=math.sqrt(5))
                nn.init.zeros_(linear_for_lora[target_name]["linear_for_lora_B"].weight)

            if lora_config.use_x != "none" and "linear_for_lora_x" not in linear_for_lora:
                if lora_config.use_x == "type1":
                    linear_for_lora["linear_for_lora_x"] = nn.Linear(in_features=in_features, out_features=lora_config.r, bias=False)
                elif lora_config.use_x == "type2":
                    linear_for_lora["linear_for_lora_x"] = nn.Linear(in_features=1, out_features=in_features, bias=False)
                nn.init.zeros_(linear_for_lora["linear_for_lora_x"].weight)
            ...
            new_module = Linear(
                ...
                bias=bias,
                linear_for_lora_A=linear_for_lora[target_name]["linear_for_lora_A"],
                linear_for_lora_B=linear_for_lora[target_name]["linear_for_lora_B"],
                linear_for_lora_x=linear_for_lora["linear_for_lora_x"] if lora_config.use_x != "none" else None,
                use_x=lora_config.use_x,
                lora_x_scaling=lora_config.lora_x_scaling,
                **kwargs
            )

        return new_module, linear_for_lora
