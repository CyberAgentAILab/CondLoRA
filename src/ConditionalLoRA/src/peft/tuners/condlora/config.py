...
@dataclass
class ConditionalLoraConfig(PeftConfig):
    """
    ...
    use_x: bool = field(
        default=False,
        metadata={
            "help": "Whether to use input example x to update initial weight."
        },
    )
    use_x: str = field(
        default="noe",
    ...
    lora_x_scaling: float = field(
        default=0.0,
        metadata={
            "help": "Scaling rate for weight calculated by input example x"
        },
    )

    def __post_init__(self):
        self.peft_type = PeftType.ConditionalLORA
